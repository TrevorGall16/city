...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [place.image],
      type: 'article',
      url: `https://citybasic.com/city/${citySlug}/${placeSlug}`,
      siteName: 'CityBasic'
    },
    twitter: {
    ...
    alternates: {
      canonical: `https://citybasic.com/city/${citySlug}/${placeSlug}`,
    },
    robots: {
      index: true,
      follow: true,
    ...
    "itemListElement": [
      {
        ...
        "position": 1,
        "name": "Home",
        "item": `https://citybasic.com/city/${citySlug}`
      },
      {
        "@type": "ListItem",
        "position": 2,
        "name": city.name,
        "item": `https://citybasic.com/city/${citySlug}/${placeSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <nav className="text-sm text-slate-600 dark:text-slate-400">
              ...
              {' / '}
              <Link href={`/city/${citySlug}`} className="hover:text-indigo-600 transition-colors">{city.name}</Link>
              {' / '}
            ...
            <Link 
              href={`/city/${citySlug}#${isFood ? 'must-eat' : 'must-see'}`}
          ...
          <div className={`grid grid-cols-1 gap-8 ${showLocationSection ? 'lg:grid-cols-3' : ''}`}>
            ...
            <div className={showLocationSection ? 'lg:col-span-2' : 'max-w-4xl mx-auto w-full'}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                  {isFood && (
                    <>
                      ...
                      {place.geo && (
                        <a
                          href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.name_en + ' ' + city.name)}`}
                          target="_blank"
              ...
              <div className="prose prose-slate dark:prose-invert max-w-none mb-8">
                {!isComplex ? (
                  ...
                  <div className="space-y-8">
                    <p className="text-xl leading-relaxed text-slate-700 dark:text-slate-200 font-medium">{desc.short}</p>
                    {desc.history && (
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 border-l-4 border-emerald-500 p-5 rounded-r-xl not-prose">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-xl">📜</span>
                          <h3 className="font-bold text-emerald-900 dark:text-emerald-200 text-sm uppercase tracking-wider m-0">History & Context</h3>
                        </div>
                        <p className="text-base text-emerald-800 dark:text-emerald-100 m-0 leading-relaxed">{desc.history}</p>
                      </div>
                    )}
                    {desc.insider_tip && (
                      <div className="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 p-5 rounded-r-xl not-prose">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-xl">💡</span>
                          <h3 className="font-bold text-blue-900 dark:text-blue-200 text-sm uppercase tracking-wider m-0">Insider Tip</h3>
                        </div>
                        <p className="text-base text-blue-800 dark:text-blue-100 m-0 leading-relaxed">{desc.insider_tip}</p>
                      </div>
                    )}
                  </div>
                )}
              ...
              {/* Cultural Staple Banner */}
              {isGenericStaple && (
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-6 mb-8">
                  <p className="text-base text-amber-900 dark:text-amber-200 leading-relaxed">
                    <span className="font-bold block mb-2 text-amber-950 dark:text-amber-100 uppercase tracking-wide text-sm">⚠️ Cultural Staple</span> 
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-slate-600 dark:text-slate-400 space-y-1">
                        <div><span className="font-medium">Coordinates:</span> {place.geo.lat.toFixed(4)}, {place.geo.lng.toFixed(4)}</div>
                        <div><span className="font-medium">City:</span> {city.name}, {city.country}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* ADSTERRA NATIVE AD */}
              <div className="my-10">
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-2">Recommended</h3>
                <AdsterraNative 
              ...
              {nearbyPlaces && nearbyPlaces.length > 0 && (
                <div className="mt-16 mb-12 border-t border-slate-200 dark:border-slate-800 pt-10">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <Link href={`/city/${citySlug}`} className="text-sm text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium">
                      View all →
                    </Link>
                  </div>
                    ...
                    {nearbyPlaces.slice(0, 3).map((item: any) => (
                      ...
                      >
                        <div className="aspect-video relative bg-slate-200 dark:bg-slate-800">
                          <Image src={item.image} alt={item.name_en} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover group-hover:scale-105 transition-transform duration-500" />
                        </div>
                        <div className="p-4">
                          <div className="font-medium text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 transition-colors line-clamp-1">{item.name_en}</div>
                          <div className="flex items-center justify-between mt-2">
                            <span className="text-xs text-slate-500 dark:text-slate-400">{item.category === 'food' ? '🍽️ Must Eat' : '🏛️ Must See'}</span>
                            {item.distance && <span className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">{item.distance.toFixed(1)} km away</span>}
                          </div>
                        </div>
                      </Link>
                    ))}
              ...
              {sameCategoryPlaces && sameCategoryPlaces.length > 0 && (
                <div className="mb-12 bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-indigo-950/20 dark:to-blue-950/20 rounded-2xl p-8 border border-indigo-100 dark:border-indigo-900">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100">More {isFood ? 'Food' : 'Attractions'} in {city.name}</h3>
                    <Link href={`/city/${citySlug}#${isFood ? 'must-eat' : 'must-see'}`} className="text-sm text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium">
                      View all {isFood ? 'restaurants' : 'attractions'} →
                    </Link>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {sameCategoryPlaces.map((item) => (
                      <Link key={item.slug} href={`/city/${citySlug}/${item.slug}`} className="group block bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden hover:border-indigo-500 dark:hover:border-indigo-500 transition-all hover:shadow-lg">
                        <div className="aspect-video relative bg-slate-200 dark:bg-slate-800">
                          <Image src={item.image} alt={item.name_en} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover group-hover:scale-105 transition-transform duration-500" />
                        </div>
                        <div className="p-4">
                          <div className="font-medium text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 transition-colors line-clamp-1">{item.name_en}</div>
                          <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">{item.name_local}</div>
                        </div>
                      </Link>
                    ))}
                  </div>
              ...
              <div className="my-12 bg-white dark:bg-slate-900 rounded-xl border-2 border-indigo-200 dark:border-indigo-800 p-6 text-center">
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-3">Ready to explore more of {city.name}?</h3>
                <Link 
                  href={`/city/${citySlug}`}
              ...
              <div className="mt-12">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-6">Community Tips & Reviews</h2>
                ...
                <div className="sticky top-24 space-y-6">
                  {isComplex && (
                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                      <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-4 text-sm uppercase tracking-wider">Quick Info</h3>
                      <div className="space-y-3 text-sm">
                        {desc.price_level && <div className="flex items-center justify-between"><span className="text-slate-600 dark:text-slate-400">Price</span><span className="font-medium">{desc.price_level}</span></div>}
                        {desc.duration && <div className="flex items-center justify-between"><span className="text-slate-600 dark:text-slate-400">Duration</span><span className="font-medium">{desc.duration}</span></div>}
                        {desc.best_time && <div className="flex items-center justify-between"><span className="text-slate-600 dark:text-slate-400">Best Time</span><span className="font-medium">{desc.best_time}</span></div>}
                      </div>
                    </div>
                  )}
                </div>
