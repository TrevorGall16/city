...
export function MonthCard({ month }: MonthCardProps) {
  return (
    <div className="bg-white rounded-xl border border-slate-100 p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 ease-out min-w-[280px]">
      {/* Header */}
      <div className="mb-4">
        <h3 className="text-lg font-bold text-slate-900">{month.name}</h3>
        <div className="text-2xl font-semibold text-indigo-600 mt-1">{month.temp}</div>
        <div className="text-sm text-slate-600 italic mt-1">{month.vibe}</div>
      </div>
      ...
      <div className="pt-3 border-t border-slate-100">
        ...
        <div className="text-sm text-slate-600">{month.clothing}</div>
      </div>
    </div>
  )
}
